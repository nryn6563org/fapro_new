  ...
  <div class="c-customer-return">
    ...
    <div class="c-customer-return__body">
      <div class="c-return-dist">
        <div v-for="(item, index) in distribution" :key="index" class="c-return-dist__item">
          ...
          <div class="c-return-dist__bar-container">
            <div class="c-return-dist__bar" :style="{ width: `${item.percent}%`, backgroundColor: item.color }"></div>
          </div>
        </div>
      </div>

      <div class="c-top-customers-container">
        <div class="c-top-customers">
          ...
          <div class="c-top-customers__list">
            <div v-for="(customer, index) in topCustomers" :key="index" class="c-top-customer-item">
        ...
        <div class="c-bottom-customers">
          <div class="c-bottom-customers__header">
            <TrendingDown :size="14" class="c-bottom-customers__icon" />
            <span class="c-bottom-customers__title">수익률 하위 TOP3</span>
          </div>
          <div class="c-bottom-customers__list">
            <div v-for="(customer, index) in bottomCustomers" :key="index" class="c-bottom-customer-item">
              <div class="c-bottom-customer-item__left">
                <span class="c-bottom-customer-item__rank">{{ index + 1 }}</span>
                <span class="c-bottom-customer-item__name">{{ customer.name }}</span>
              </div>
              <span class="c-bottom-customer-item__value">{{ customer.return }}%</span>
            </div>
          </div>
        </div>
      </div>
...
<script>
import { PieChart, RefreshCw, TrendingUp, TrendingDown } from "lucide-vue";

export default {
  name: "PageDashboardCustomerReturn",
  components: { PieChart, RefreshCw, TrendingUp, TrendingDown },
  data() {
    return {
      distribution: [
      ...
      bottomCustomers: [
        { name: "이비룡", return: -15.2 },
        { name: "최배달", return: -12.4 },
        { name: "강건마", return: -11.8 }
      ]
    };
  }
};
