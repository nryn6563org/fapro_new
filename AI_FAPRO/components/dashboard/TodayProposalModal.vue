<template>
  <ContactProposalModal
    ...
    :single-customer="proposalData.customer || proposalData"
    :single-stock="proposalData.stock"
...
/**
 * TodayProposalModal
 * 설명: 대시보드의 다양한 제안 버튼(오늘 타켓 고객, 매수/매도 종목, 스마트 고객관리)에 대응.
  ...
  methods: {
    initData() {
      ...
      if (this.proposalType === 'target-customer') {
        this.internalStocks = [
          { name: '펄어비스', ticker: '263750', price: '42,500', change: '+7.19%', reason: '매수 타이밍입니다.' },
          { name: '카카오게임즈', ticker: '293490', price: '34,850', change: '-2.3%', reason: '성장성이 기대됩니다.' }
        ]
        this.checkedTickers = this.internalStocks.map(s => s.ticker)
      } else if (this.proposalType === 'target-stock') {
        this.internalCustomers = [
          { id: 1, name: '김철수', email: 'chulsoo@example.com', portfolio: '15억', investmentStyle: '공격투자형' },
          { id: 2, name: '이영희', email: 'younghee@example.com', portfolio: '8억', investmentStyle: '안정추구형' }
        ]
        ...
        this.internalStocks = [
          { name: '알테오젠', ticker: '196170', price: '285,000', change: '+6.65%', reason: '추종 매수가 유효합니다.' }
        ]
        this.checkedTickers = this.internalStocks.map(s => s.ticker)
      }
    },
