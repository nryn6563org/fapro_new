 ...
 * 설명: 5가지 제안 디자인을 모두 지원하는 통합 기획/제안 모달
 */
...
import '~/assets/css/components/proposal/ProposalComponents.css'
...
export default {
  ...
  computed: {
    config() {
      const modeConfigs = {
        'today-customer': {
          title: '투자 제안',
          description: '타겟 고객에게 문자, 카카오톡 또는 이메일로 제안 합니다.',
          iconType: 'send',
          theme: 'teal',
  ...
  watch: {
    mode: {
      handler() { this.generateMessage() },
      immediate: true
    },
    internalDeliveryMethod() { this.generateMessage() },
    selectedCustomerIds() { this.generateMessage() },
    selectedTickers() { this.generateMessage() },
    initialMessage(newVal) { this.internalMessage = newVal },
    internalSelectedCustomerIds(newVal) { this.$emit('update:selectedCustomerIds', newVal) },
    internalSelectedTickers(newVal) { this.$emit('update:selectedTickers', newVal) }
  },
  methods: {
    generateMessage() {
      // 1. Basic Info
      const customerName = this.singleCustomer?.name || '고객'
      const stocksText = this.internalSelectedTickers.length > 0 
        ? this.stocks.filter(s => this.internalSelectedTickers.includes(s.ticker)).map(s => s.name).join(', ')
