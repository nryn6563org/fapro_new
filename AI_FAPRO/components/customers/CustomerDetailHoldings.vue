      ...
      <table class="customer-holdings__table">
        ...
        <tbody class="customer-holdings__tbody">
          ...
          >
            ...
            <td class="customer-holdings__td">
              <div class="flex items-center gap-1 justify-end">
                <trending-up-icon
                  v-if="holding.profit && holding.profit.includes('+')"
                  class="w-3.5 h-3.5 text-emerald-500"
                />
                <trending-down-icon
                  v-else-if="holding.profit && holding.profit.includes('-')"
                  class="w-3.5 h-3.5 text-blue-500"
                />
                <span :class="getProfitColor(holding.profit)">{{
                  holding.profit || "-"
                }}</span>
              </div>
            </td>
            <td class="customer-holdings__td">
              <span :class="getProfitColor(holding.profitRate)">{{
                holding.profitRate || "-"
              }}</span>
            </td>
          </tr>
          <tr v-if="!holdings || holdings.length === 0">
            <td
              colspan="7"
              class="customer-holdings__td customer-holdings__td--empty"
            >
              보유 종목이 없습니다.
            </td>
          </tr>
