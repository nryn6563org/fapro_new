    ...
    <div class="schedule-sync-modal__box animate__animated animate__fadeIn animate__faster">
      ...
      <div v-if="mode === 'sync'" class="schedule-sync-modal__body">
        <div class="schedule-sync-modal__header">
          <div class="schedule-sync-modal__header-content">
            <div
              class="schedule-sync-modal__icon-box schedule-sync-modal__icon-box--teal"
            >
          ...
          <button
            class="schedule-sync-modal__close-btn"
            @click="$emit('close')"
          >
        ...
        <div class="schedule-sync-modal__section">
          ...
          <ul class="schedule-sync-modal__list">
            <li
              ...
              :key="i"
              class="schedule-sync-modal__list-item"
            >
        ...
        <div class="schedule-sync-modal__footer">
          <button
            class="schedule-sync-modal__btn-cancel"
            @click="$emit('close')"
          >
            취소
          </button>
          <button
            class="schedule-sync-modal__btn-primary"
      ...
      <div v-if="mode === 'unsync'" class="schedule-sync-modal__body">
        <div class="schedule-sync-modal__header">
          <div class="schedule-sync-modal__header-content">
            <div
              class="schedule-sync-modal__icon-box schedule-sync-modal__icon-box--red"
            >
              <alert-triangle-icon
                class="schedule-sync-modal__main-icon text-red-600 dark:text-red-500"
              />
            </div>
        ...
        <div class="schedule-sync-modal__section">
          <p class="schedule-sync-modal__description-bold">
            동기화를 해제하시겠습니까?
          </p>
          <div class="schedule-sync-modal__alert">
            <p class="schedule-sync-modal__alert-text m-0">
              동기화를 해제하면 실시간 일정 업데이트 및 자동 동기화 기능이
              비활성화됩니다.
            </p>
          </div>
        </div>
        <div class="schedule-sync-modal__footer">
          ...
          <button
            class="schedule-sync-modal__btn-danger"
            @click="$emit('confirm-unsync')"
          >
            동기화 해제
          </button>
        </div>
      ...
      <div
        v-if="mode === 'syncing'"
        class="schedule-sync-modal__body text-center"
      >
        <div class="schedule-sync-modal__loader-box">
          <loader-icon class="schedule-sync-modal__loader-icon" />
        </div>
        <h2 class="schedule-sync-modal__title schedule-sync-modal__title--mb-2">
          Google Calendar 동기화 중
        </h2>
        <p class="schedule-sync-modal__loading-text">
          잠시만 기다려주세요. 일정을 동기화하고 있습니다...
        </p>
      </div>
    </div>
  </div>
