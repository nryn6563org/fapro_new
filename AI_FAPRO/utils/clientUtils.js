...
export const extractClientInfoFromText = (text) => {
  ...
  // 나이 추출
  const ageMatch = text.match(/나이[:\s]*(\d{2})|연령[:\s]*(\d{2})|(\d{2})\s*세/)
  if (ageMatch) {
    extracted.age = parseInt(ageMatch[1] || ageMatch[2] || ageMatch[3])
  }

  // 투자유형 추출
  if (text.includes('공격형') || text.includes('공격투자') || text.includes('적극')) {
    extracted.investmentType = '공격형'
  } else if (text.includes('안정형') || text.includes('안정투자') || text.includes('보수')) {
    extracted.investmentType = '안정형'
  } else if (text.includes('중립형') || text.includes('균형') || text.includes('중도')) {
  ...
  if (assetMatch) {
    extracted.totalAssets = (assetMatch[1] || assetMatch[2] || assetMatch[3]).trim()
  }
  ...
  // 특이사항/메모 추출
  const notesMatch = text.match(/특이사항[:\s]*([^\n]+)|메모[:\s]*([^\n]+)|비고[:\s]*([^\n]+)/)
  if (notesMatch) {
